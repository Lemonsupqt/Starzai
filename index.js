...
// APIMart Image/Video Generation (SeedDream 4.5 + future models)
import {
  APIMartClient,
  APIMART_MODELS,
  APIMART_ASPECT_RATIOS,
  getAvailableModels as getAPIMartModels,
  getImageModels as getAPIMartImageModels,
  getModelConfig as getAPIMartModelConfig,
  getDefaultImageModel as getAPIMartDefaultModel,
  getAspectRatioConfig as getAPIMartRatioConfig,
  parseAspectRatio as parseAPIMartRatio,
...
// APIMart for SeedDream 4.5 + future models (image & video generation)
// Set APIMART_API_KEY in Railway variables
const APIMART_API_KEY = process.env.APIMART_API_KEY || "";
const apimartClient = new APIMartClient(APIMART_API_KEY);
if (APIMART_API_KEY) {
  console.log(`[APIMart] Initialized with API key (${APIMART_API_KEY.slice(0, 8)}...)`);
} else {
  console.log(`[APIMart] No API key configured. Set APIMART_API_KEY in Railway variables.`);
}
...
function ensureUser(userId, from = null) {
  ...
  if (!usersDb.users[id]) {
    ...
    usersDb.users[id] = {
      ...
      // APIMart art generation preferences (/a, /ass)
      artPrefs: {
    ...
    // migration: add artPrefs for APIMart /a settings if missing
    if (!usersDb.users[id].artPrefs) {
...
bot.callbackQuery(/^mdl:/, async (ctx) => {
  ...
  try {
    ...
    // Store audio message id so lyrics can reply to the song
    const bufData = pendingMusicBuffers.get(dlKey);
    if (bufData) {
      bufData.audioMessageId = audioMsg.message_id;
      bufData.chatId = ctx.chat.id;
    }
...
// Callback: show lyrics for downloaded song
bot.callbackQuery(/^mlyrics:/, async (ctx) => {
  const dlKey = ctx.callbackQuery.data.slice(8);
  const data = pendingMusicBuffers.get(dlKey);

  if (!data?.song) {
    return ctx.answerCallbackQuery({ text: '⏰ Session expired. Use /lyrics to search.', show_alert: true });
  }

  const { song, audioMessageId, chatId } = data;
  await ctx.answerCallbackQuery({ text: '🎵 Fetching lyrics...' });
  const replyToId = audioMessageId || ctx.callbackQuery?.message?.message_id;

  try {
    const query = `${song.artist} ${song.name}`.trim();
    const result = await searchLyricsLRCLIB(query);

    if (!result.success || (!result.lyrics && !result.instrumental)) {
      // Try with just the song name
      const result2 = await searchLyricsLRCLIB(song.name);
      if (!result2.success || (!result2.lyrics && !result2.instrumental)) {
        return ctx.reply(
          `❌ Lyrics not found for "${escapeHTML(song.name)}"\n\n` +
          `<i>Try manually with</i> <code>/lyrics ${escapeHTML(song.name)}</code>`,
          { parse_mode: 'HTML' }
        );
      }
    ...
    // Beautiful lyrics formatting
    let lyrics = result.lyrics;
    const hasSynced = !!result.syncedLyrics;
    const hasVersions = (result.allResults?.length || 0) > 1;
    const dur = result.duration ? `${Math.floor(result.duration / 60)}:${String(Math.floor(result.duration % 60)).padStart(2, '0')}` : '';
    ...
    // Store for synced/versions callbacks
    const buttons = [];
    if (hasSynced || hasVersions) {
      pendingLyricsResults.set(ctx.from.id, {
        allResults: result.allResults || [],
        currentIndex: 0,
      ...
      if (hasSynced) buttons.push({ text: '⏱ Synced Lyrics', callback_data: `lyrics_synced:${ctx.from.id}` });
    ...
    await ctx.reply(
      ...
      { parse_mode: 'HTML', reply_parameters: replyToId ? { message_id: replyToId } : undefined }
    );
  }
});

// Callback: show lyrics from inline music result
bot.callbackQuery(/^milyrics:/, async (ctx) => {
  const key = ctx.callbackQuery.data.slice(9);
  const data = inlineCache.get(key);

  if (!data?.song) {
    return ctx.answerCallbackQuery({ text: '⏰ Session expired. Use /lyrics to search.', show_alert: true });
  }

  if (String(ctx.from.id) !== String(data.userId)) {
    return ctx.answerCallbackQuery({ text: '⚠️ This button is not for you.', show_alert: true });
  }
  ...
  try {
    const query = `${song.artist} ${song.name}`.trim();
    let result = await searchLyricsLRCLIB(query);
...
bot.command("qr", async (ctx) => {
  ...
  if (artStyle === "v1" || artStyle === "v2") {
    ...
    effectiveLevel = getEffectiveQrErrorCorrection(
      errorCorrectionLevel,
      needsHighEc
    );
  }
...
// Engine 1: Real-ESRGAN (bookbot) — fast, CPU-based, always on
...
const UPSCALE_MODELS = {
  ...
  "esrgan_2x":    { name: "2x Fast",       icon: "⚡", scale: 2, engine: "esrgan", esrganModel: "modelx2" },
  "esrgan_4x":    { name: "4x Fast",       icon: "⚡", scale: 4, engine: "esrgan", esrganModel: "modelx4" },
...
// ─── Real-ESRGAN upscaler (bookbot Space, Gradio 3.x) ───
async function upscaleESRGAN(imageBuffer, esrganModel) {
  const base64 = `data:image/png;base64,${imageBuffer.toString('base64')}`;
  const resp = await fetch(ESRGAN_SPACE_URL + "/api/predict", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ data: [base64, esrganModel] }),
  });
  if (!resp.ok) throw new Error(`ESRGAN API returned ${resp.status}`);
  const result = await resp.json();
  if (result.error) throw new Error(result.error);
  const outputBase64 = result.data?.[0];
  if (!outputBase64 || typeof outputBase64 !== 'string' || !outputBase64.startsWith('data:')) {
    throw new Error('Invalid ESRGAN response');
  }
  return Buffer.from(outputBase64.split(',')[1], 'base64');
}

// ─── AuraSR-v2 upscaler (GigaGAN, Gradio 4.x queue API) ───
async function upscaleAuraSR(imageBuffer) {
  ...
  const uploadResp = await globalThis.fetch(AURASR_SPACE_URL + "/upload", {
    method: "POST",
    body: formData,
  });
  if (!uploadResp.ok) throw new Error(`AuraSR upload failed: ${uploadResp.status}`);
  const uploadResult = await uploadResp.json();
  const filePath = Array.isArray(uploadResult) ? uploadResult[0] : uploadResult;
  
  // Step 2: Join queue
  const joinResp = await globalThis.fetch(AURASR_SPACE_URL + "/queue/join", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      data: [{ path: filePath, url: AURASR_SPACE_URL + "/file=" + filePath, orig_name: "image.jpg", size: imageBuffer.length, mime_type: "image/jpeg", meta: { _type: "gradio.FileData" } }],
      fn_index: 0,
      session_hash: sessionHash,
    }),
  });
  if (!joinResp.ok) throw new Error(`AuraSR queue join failed: ${joinResp.status}`);
  
  // Step 3: SSE polling for result
  const dataResp = await globalThis.fetch(AURASR_SPACE_URL + "/queue/data?session_hash=" + sessionHash, {
...
async function upscaleImage(imageBuffer, model = DEFAULT_UPSCALE_MODEL) {
  ...
  if (!info) throw new Error(`Unknown upscale model: ${model}`);
