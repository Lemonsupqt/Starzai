...
bot.command("ownerhelp", async (ctx) => {
  ...
  const lines = [
    ...
    "• Shorter/Longer & Ultra Summary behavior depends on user tier (see Plans in menu)",
    "",
    "🚫 *Bans*",
    ...
    "",
    "👥 *Groups & anti-spam*",
    "• Groups must be authorized with `/allowgroup <chatId>` before normal use",
    "• In authorized groups, bot replies when mentioned, when message contains \"Starz\"/\"StarzAI\", when users reply to the bot, or when a character is active",
    "• Use `/talk` to temporarily make the bot respond to all messages in a group",
    "• Use `/stop` to return to mention / wake-word only mode",
    "",
